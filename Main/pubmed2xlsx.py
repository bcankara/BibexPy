...
def labelling(data: pd.DataFrame) -> pd.DataFrame:
    """Converts PubMed column labels to standard format"""
...
def csvPubMed2df(files: Union[str, List[str]]) -> pd.DataFrame:
    """
    Reads and processes PubMed CSV files
...
def save_to_excel(file_paths: Union[str, List[str]], output_path: str) -> bool:
    """
    Processes PubMed CSV files and saves as Excel file
...
if __name__ == "__main__":
    try:
        ...
        while True:
            print("\nEnter the full path or file names of PubMed CSV files separated by commas")

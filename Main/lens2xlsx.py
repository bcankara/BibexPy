...
def labelling(data: pd.DataFrame) -> pd.DataFrame:
    """Converts Lens column labels to standard format"""
...
def process_pages(row: pd.Series) -> str:
    """Process start and end pages to create PP field"""
    if pd.isna(row.get('BP', '')) or pd.isna(row.get('EP', '')):
        return ''
    
    try:
        start = str(int(float(row['BP'])))
        end = str(int(float(row['EP'])))
        return f"{start}-{end}"
    except:
        return ''


def csvLens2df(files: Union[str, List[str]]) -> pd.DataFrame:
    """
    Reads and processes Lens CSV files
    ...
    # Process pages
    if 'BP' in DATA.columns and 'EP' in DATA.columns:
        DATA['PP'] = DATA.apply(process_pages, axis=1)
...
def save_to_excel(file_paths: Union[str, List[str]], output_path: str) -> bool:
    """
    Processes Lens CSV files and saves as Excel file
...
if __name__ == "__main__":
    try:
        ...
        while True:
            print("\nEnter the full path or file names of Lens CSV files separated by commas")

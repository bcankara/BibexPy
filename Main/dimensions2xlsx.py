...
def labelling(data: pd.DataFrame) -> pd.DataFrame:
    """Converts Dimensions column labels to standard format"""
    
    df_tag = pd.DataFrame([
        ["Source title/Anthology title", "SO"],
        ["Title", "TI"],
        ["Abstract", "AB"],
        ["Authors", "AU"],
        ...
        ["Publisher", "PU"],
        ["Language", "LA"],
        ["Authors Affiliations Name of Research organization", "AU_UN"],
        ["Authors Affiliations Country of Research organization", "AU_CO"]
    ], columns=['orig', 'tag'])
...
def process_citations(citations: str) -> str:
    """Process citation string from Dimensions format to standard format"""
    if pd.isna(citations) or citations == '':
        return ''
        
    refs = []
    for ref in citations.split(';['):
        parts = ref.replace('|', '!!!').split('!!!')
        if len(parts) >= 8:
            # Extract author
            authors = parts[0].replace('[', '').split(';')
            if authors:
                author = authors[0].strip()
                lastname = re.sub(r',.*', '', author).strip()
                firstname = re.sub(r'.*,', '', author).strip().split()
                if firstname:
                    initials = ''.join(n[0] for n in firstname)
            ...
            year = parts[3] if len(parts[3]) > 0 else "0000"
            journal = parts[2] if len(parts[2]) > 0 else "NA"
            volume = f"V{parts[4]}" if len(parts[4]) > 0 else ""
            issue = f"N{parts[5]}" if len(parts[5]) > 0 else ""
            doi = parts[7] if len(parts[7]) > 0 else ""
            
            ref_str = ", ".join(filter(None, [author, year, journal, volume, issue, doi]))
            refs.append(ref_str)
            
    return ";".join(refs)


def csvDimensions2df(files: Union[str, List[str]]) -> pd.DataFrame:
    """
    Reads and processes Dimensions CSV files
    ...
    for i, file in enumerate(files):
        try:
            ...
            df = pd.read_csv(file,
                           ...
                           encoding='utf-8',
                           skip=1)  # Skip header row as per R implementation
    ...
    if 'CR' in DATA.columns:
        DATA['CR'] = DATA['CR'].apply(process_citations)
...
def save_to_excel(file_paths: Union[str, List[str]], output_path: str) -> bool:
    """
    Processes Dimensions CSV files and saves as Excel file
    ...
    try:
        # Convert data
        df = csvDimensions2df(file_paths)
...
if __name__ == "__main__":
    try:
        ...
        while True:
            print("\nEnter the full path or file names of Dimensions CSV files separated by commas")
